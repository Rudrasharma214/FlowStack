...
import {
  generateInviteToken,
  verifyInviteToken,
} from '../../utils/inviteToken.utils.js';
...
export class ProjectMemberService {
  ...
  async verifyInvitation(token) {
    try {
      ...
      const user = invitation.user_id
        ? await userRepository.findById(invitation.user_id)
        : null;
      if (!user) {
        return {
          success: true,
  ...
  async acceptInvitation(token) {
    ...
    try {
      ...
      await projectInvitationRepository.updateInvitationStatus(
        invitation.id,
        'accepted',
        new Date(),
        transaction
  ...
  async removeMember(projectId, memberId) {
    try {
      const result = await projectMemberRepository.removeProjectMember(
        projectId,
        memberId
      );
