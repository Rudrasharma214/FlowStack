...
export class ProjectMemberRepository {
    ...
    async getProjectMembersByProjectId({
        ...
        const { count, rows } = await ProjectMember.findAndCountAll({ where: whereClause, include, transaction, ...options });
        return { count, rows };
    }

    /* Remove a Project Member */
    async removeProjectMember(projectId, userId, transaction = null) {
        return await ProjectMember.destroy({ where: { projectId, userId }, force: true, transaction });
    }
};