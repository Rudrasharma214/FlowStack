...
export class AuthService {
    ...
    async verifyEmail(token) {
        ...
        try {
            ...
            const record = await verifyTokenRepository.findByUserIdAndType(
                userId,
                'email_verification',
                transaction
            );
    ...
    async login(email, password) {
        ...
        try {
            ...
            const daysSinceLastLogin = lastLogin
                ? Math.floor(
                    (currentDate - new Date(lastLogin)) / (1000 * 60 * 60 * 24)
                )
                : null;
    ...
    async verifyLoginOTP(userId, otp) {
        ...
        try {
            ...
            const otpRecord = await otpRepository.findByUserIdAndCode(
                userId,
                otp,
                transaction
            );
        ...
        try {
            ...
            if (!user) {
                ...
                return {
                    success: false,
                    message:
            'If user exists, a password reset link will be sent to the email.',
    ...
    async resetPassword(token, password) {
        ...
        try {
            ...
            await user.update({ password: passwordHashed }, { transaction });
